...
export interface UserRepository {
   ...
   * 原子增减用户统计（并发安全，不丢更新）
   * @param id 用户ID
   * @param field 统计字段（articles/likes/views）
   * @param delta 增量，可正可负
   * @returns 是否更新成功；用户不存在返回 false
   */
  incrementStats(
    id: string,
    field: 'articles' | 'likes' | 'views',
    delta: number,
  ): Promise<boolean>;
  /**
   * 原子切换用户与文章的关联（点赞/收藏列表增删，并发安全）
   * @param id 用户ID
   * @param field 关联列表字段（likedArticles / favoritedArticles）
   * @param postId 文章ID
   * @param onToggle 可选回调，入参为切换前该文章是否已在列表中
   * @returns 是否切换成功；用户不存在返回 false
   */
  toggleAssociation(
    id: string,
