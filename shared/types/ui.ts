...
/**
 * 容器组件 Props（复用 {@link ChildrenProps}）
 */
...
export interface RequestOptions extends Omit<RequestInit, 'body' | 'cache'> {
  ...
  /** 服务端跳过 httpOnly Cookie 转发（公开数据专用：不触发 cookies() 动态 API，
   *  使页面可静态渲染/ISR，同时避免带 Cookie 的响应进入共享缓存） */
  skipAuth?: boolean;
   /** fetch 缓存策略（RequestCache 来自 DOM 标准，Next.js 扩展 'only-cache' 需 cast） */
  cache?: RequestCache;
}
