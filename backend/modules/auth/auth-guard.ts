 ...
 *
 * 不依赖任何 Web 框架：入参只要求「能按名字读 Cookie」的最小结构。
 * Next 适配：NextRequest 原生满足；Express 适配：包一层 `{ cookies: { get: (n) => ({ value: req.cookies[n] }) } }`。
 */
...
/** 能按名字读取 Cookie 的最小请求结构（框架无关） */
export interface CookieReadable {
  /** 按 Cookie 名取值的读取器；不存在时返回 undefined */
 ...
 */
export async function requireAuth(request: CookieReadable, deps: AuthDeps): Promise<AuthPayload> {
