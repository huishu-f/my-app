...
import type { BlogRepository } from '../kv-blog-repository';
import type { UserRepository } from '../../auth/kv-user-repository';
import type { CommentRepository } from '../../comment/kv-comment-repository';
import type {
  CreatePostDto,
  ListPostsOptions,
  PostsListData,
...
export function createBlogService(deps: {
   ...
   * 响应后执行器（框架注入，可选）。
   *
   * 语义：把 task 排到「当前请求响应已返回之后」再执行，避免阻塞响应，同时保证进程不在
   * 任务完成前被回收。Next 适配传 `after`（serverless 响应后函数会被冻结，
   * 裸 fire-and-forget 的写入会被静默丢弃）；Express 长驻进程可直接用默认实现。
   *
   * 缺省为裸 fire-and-forget，并由调用方自带的 catch 兜底。
   */
  scheduleAfterResponse?: (task: () => Promise<void>) => void;
}): BlogService {
  /** 响应后执行器：未注入时退化为 fire-and-forget（Express 等长驻进程安全） */
  const schedule = deps.scheduleAfterResponse ?? ((task: () => Promise<void>) => void task());
  /**
   * 分页查询文章列表
  ...
  async function listPosts(options: ListPostsOptions): Promise<PostsListData> {
    ...
    // 按作者过滤：个人中心「我的文章」用，避免把全站文章拉给客户端再自行筛
  ...
  async function incrementView(id: string): Promise<void> {
    ...
    // schedule：响应返回后执行写入（Next 传 after，Express 走默认 fire-and-forget），
    // 替代裸 fire-and-forget——后者在 Next serverless 下响应后函数可能被冻结/回收，写入被静默丢弃
    schedule(async () => {
      try {
