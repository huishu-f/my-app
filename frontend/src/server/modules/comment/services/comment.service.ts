...
/**
 * 创建评论 service，接收 repository 依赖，返回一组评论业务操作方法
...
export function createCommentService(deps: {
  ...
  /**
   * 查询指定文章下的评论列表，按创建时间降序排列；校验文章存在性：不存在的文章抛 NotFoundError（404），
   * 避免返回误导性的空列表导致客户端无法区分“无评论”与“文章不存在”；
   * 草稿文章仅作者本人可见，与详情页公开视角 404 的语义对齐，防止匿名用户通过该接口探知草稿下的评论内容
   * @param options 查询选项，包含 postId 与后端注入的认证上下文
   * @returns 评论列表
   */
  ...
  /**
   * 创建评论，校验文章和用户存在性后写入评论，并原子更新文章评论计数，失败时回滚已创建的评论
  ...
  /**
   * 编辑评论内容，校验评论存在性和操作权限后更新评论内容和更新时间
   * @param id 评论 ID
  ...
  /**
   * 删除评论，校验评论存在性和操作权限（评论作者或文章作者）后删除评论，并原子递减文章评论计数
   * @param id 评论 ID
   * @param currentUserId 当前用户 ID
