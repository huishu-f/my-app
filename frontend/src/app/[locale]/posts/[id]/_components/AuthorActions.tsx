 ...
 * @description 作者专属操作（编辑/删除）客户端岛屿。
 *              详情页公开内容静态化后服务端渲染不再判权（快路径无登录态），
 *              作者身份由客户端 useAuth 实时判定，SSR user prop 仅作初始兜底。
 */
'use client';
...
/**
 * AuthorActions 作者操作栏组件
 * 仅当当前登录用户即文章作者时渲染删除按钮组，否则返回 null
 * @param props.postId 文章 ID
 * @param props.authorId 文章作者 ID
 * @param props.ssrUser SSR 传入的用户（慢路径草稿预览时有值），作客户端登录态的初始兜底
 */
