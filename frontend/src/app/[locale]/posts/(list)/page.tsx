...
import { PinnedBadge } from '@/ui/PinnedBadge';
import { PageHeader } from '@/ui/layout/PageHeader';
import { listPostsServer, getCategoriesServer, getTagsServer } from '@/services/blog/load';
...
import { PostsSearchInput } from '@/features/posts/components/PostsSearchInput';
 ...
 * 搜索词长度上限（字符）。
 *
 * `?q=` 是用户在地址栏可任意构造的无界字符串，而本页按「完整 URL」生成 Full Route Cache 条目 ——
 * 不设上限意味着任意查询串都能让 CDN/缓存层多出一份条目，且每个新词都触发一次完整渲染。
 * 这里用截断而非报错：既能挡住缓存膨胀，又让超长输入退化成「前 N 字符的搜索结果」，对用户仍是可用结果。
 */
...
export default async function PostsPage({
  ...
  const q =
    typeof sp.q === 'string'
      ? sp.q.trim().slice(0, MAX_SEARCH_QUERY_LENGTH) || undefined
      : undefined;
