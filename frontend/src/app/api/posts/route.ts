...
import { sendSuccess, sendCreated, publicCacheHeaders } from '@/server/api-response';
import { parseCreatePostBody, parseListQuery } from '@my-app/backend/modules/blog/blog-validators';
...
export const POST = defineRoute(
  ...
  {
    auth: 'required',
    // 与 createPostAction 共用同一份策略（含同一个计数键），两处不会各自漂移
    rateLimit: POST_CREATE_RATE_LIMIT,
  },
);
