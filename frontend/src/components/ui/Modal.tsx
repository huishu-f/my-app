...
export function Modal({ open, onClose, title, children, maxWidth = 'max-w-sm' }: ModalProps) {
  ...
  return (
    ...
    >
      ...
      ref={dialogRef}
      tabIndex={-1}
        className={`w-full ${maxWidth} border-card-border bg-card-bg animate-fade-in rounded-2xl border p-8 shadow-lg focus:outline-none`}
      ...
      >
        ...
        {title && (
          <div className="row-md mb-7 justify-between">
            ...
            <button
              onClick={onClose}
              className="text-faint hover:bg-stroke hover:text-heading inline-flex h-9 w-9 items-center justify-center rounded-lg transition-colors duration-200"
              aria-label="关闭"
            >
