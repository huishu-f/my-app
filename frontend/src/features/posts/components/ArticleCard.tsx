 ...
 * @description 文章卡片组件：横向/纵向两种布局展示封面、分类、标题、摘要、作者与统计；整卡可点击跳转，支持角标、标签、操作区与扩展统计插槽
 */
...
import { stripHtml, stripMarkdown } from '@my-app/shared/lib/markdown';
import { isSafeImageUrl } from '@my-app/shared/lib/validators';
import type { ArticleCardProps } from '@my-app/shared';

/**
 * ArticleCard 文章卡片
 * 用 memo 包裹：列表大量渲染时避免父级更新带来的无谓重绘。
 * @param props {@link ArticleCardProps}（Props 类型由 @my-app/shared 提供，字段含义见该处）
 */
export const ArticleCard = memo(function ArticleCard({
  ...
  /** 分类展示文案：已知分类走 i18n，未知分类回退后端原始字符串 */
  ...
  /** 封面区块：仅当有地址且通过 isSafeImageUrl 校验时用 next/image，否则降级占位图 */
  const cover = (
    <div
    ...
    >
      {coverImage && isSafeImageUrl(coverImage) ? (
        <Image
          ...
          className="ease-smooth aspect-16/10 w-full rounded-md object-cover transition-transform duration-300 group-hover:scale-103"
        />
      ) : (
  ...
  /** 按 variant 组装卡片主体：纵向竖卡布局 / 横向列表卡布局 */
  ...
  return (
    <div className={baseClass}>
      {/* 透明整卡 Link 层承担跳转（--z-content）；卡片内标签/操作区用 --z-raised 抬到其之上，保证仍可点击 */}
      {href && (
        <Link
          href={href}
