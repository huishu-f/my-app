...
import { RemoveFavoriteButton } from '@/features/profile/components/RemoveFavoriteButton';
...
interface ProfileTabsProps {
  /** 我已发布的文章列表；null 表示服务端取数失败 */
  published: Post[] | null;

  /** 我的草稿列表；null 表示服务端取数失败 */
  drafts: Post[] | null;

  /** 我的收藏列表；null 表示服务端取数失败 */
  favorites: Post[] | null;
}
 ...
 * 列表加载失败态
 *
 * 与「确实没有内容」是两种不同状态：服务端取数失败时拿到 null，这里给错误态与重试入口，
 * 而不是显示「还没有文章」让用户以为自己没写过。
 */
 ...
 */
export function ProfileTabs({ published, drafts, favorites }: ProfileTabsProps) {
  const t = useTranslations('profile');
  ...
  return (
    <div className="min-w-0">
      <div className="segmented animate-fade-in">
        ...
        >
          {t('articlesTab', { count: published?.length ?? 0 })}
        </button>
        <button
          type="button"
